...
//			printf("\n******************\n");
		...
		if (recive_flag) //缓冲区有数据
			...
			if (outmode == UARTbluetooth) //串口助手调试输出格式
				...
				printf("d:%.2f et3:%d et1:%d et2:%d\n", d_distance,err_time,time_data.err_time1,time_data.err_time2);
...
//                (measure_data_arr[i_gen].reference_index[3] -measure_data_arr[i_gen].reference_index[2]) *refclk_divisions;
			...
			case line: //测线模式数据筛选
				...
				else if(err_time3>5000 && (1.5 * (double) time_ps) / 10000 <100)
					{
						data_vaild= 0;
					}
				break;
		...
		if(vaild_data_num == 0)		//该次测量没有效数据,重新测量
			{
				if(!first_measure && mcb.measure_mode==wall)	//第一次测量			问：如果测线第一没对准咋整？？？
				{
						OS_ENTER_CRITICAL();
						printf("exit success 1600.\r\n");
						OS_EXIT_CRITICAL();
						first_measure=1;
		                continue;
				}
		...
		else if(vaild_data_num!=0 && !first_measure)
			{
				first_measure=1;
				OS_ENTER_CRITICAL();
				quantify			= LEVEL[0]; 			//设置q:1100
				printf("exit success 1100.\r\n");
				setDacValueBin(quantify);
				OS_EXIT_CRITICAL();
				continue;	//该次测量数据丢弃
			}
...
//										printf("D:",distance);
				...
				//						printf("D:",distance);
				...
				else	//看看有没有,有再说
					{
						...
						if(quantify>1500 && vaild_Measuredata[1].distance <100)
						{
							OS_ENTER_CRITICAL();
							quantify			= LEVEL[0]; 			//设置q:1100
						...
						{
							if(data_2_filter(old_2relt_timedata,Data2_measure_cycle,&time_data))	//数据有效
								{
									OS_ENTER_CRITICAL();					//ans_buf	ans_cnt  ans_ite 为临界资源
									if (ans_cnt < ANS_BUF_SIZE)
										{
